  ...
  */
  ...
  $(document).ready(function() {
    ...
    function showPage(page) {
        ...
        $('.section-line').hide();
        $('.section-line').slice(startIndex, endIndex - 1).show();
  ...
  $(document).ready(function() {
    // Opções para cada tipo de transação
    const options = {
        Comprar: [
            {value: "casa", text: "Casa"},
            {value: "apartamento", text: "Apartamento"},
            {value: "terreno", text: "Terreno"},
            {value: "comercial", text: "Imóvel Comercial"},
            {value: "rural", text: "Propriedade Rural"}
        ],
        Alugar: [
            {value: "casa", text: "Casa"},
            {value: "apartamento", text: "Apartamento"},
    ...
    // Função para atualizar as opções de busca
    function updateSearchOptions(transactionType) {
        const $searchKeyword = $('#searchKeyword');
        $searchKeyword.empty(); // Limpa as opções atuais
        $searchKeyword.append($('<option>', {
            value: '',
            text: 'O que você procura?',
            hidden: true
        }));
        options[transactionType].forEach(function(option) {
            $searchKeyword.append($('<option>', {
                value: option.value,
                text: option.text
            }));
    ...
    // Evento para mudança no tipo de transação
    $('input[name="type"]').on('change', function() {
        updateSearchOptions(this.value);
    });

    // Inicializa com as opções de "Comprar"
    updateSearchOptions('Comprar');
    ...
    $('#searchForm').on('submit', function(e) {
        ...
        if (keyword === '' && city === '') {
            showError('Por favor, selecione o que você está procurando e uma cidade.');
            return;
        } else if (keyword === '') {
            showError('Por favor, selecione o que você está procurando.');
            return;
        } else if (city === '') {
        ...
        var url = city + '/' + type.toLowerCase() + '.html';
        url += '?keyword=' + encodeURIComponent(keyword);

        window.location.href = url;
    });

    // Esconde a mensagem de erro quando o usuário interage com os campos
    $('#searchKeyword, #city').on('change', function() {
        $('#errorMessage').hide();
    });
});
